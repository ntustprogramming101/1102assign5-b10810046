...
void draw() {
  ...
  case GAME_RUN: // In-Game
    ...
    for (int i = 0; i < soldierX.length; i++) {
      ...
      // Requirement #3: Use boolean isHit(...) to detect collision
      if (isHit(playerX, playerY, playerX + SOIL_SIZE, playerY + SOIL_SIZE, soldierX[i], soldierY[i], soldierX[i] + SOIL_SIZE, soldierY[i] + SOIL_SIZE)) { // r1 bottom edge past r2 top
...
int getEnemyIndexByRow(int row) {				// Requirement #
  for (int i =0; i <= 5; i++) {
    int soldierRow = (int)soldierY[i]/SOIL_SIZE;
    if ( soldierRow == row ) {
      return i;
    }
  }
